...
/**
 * @var string - Define qual será o código utilizado para compor o "nosso número":
 *                  Caso seja CLI, nosso número será composto por: $codigo_cliente$mes$ano;
 *                  Caso seja TIT, nosso número será composto por: $num_doc$mes$ano;
 */
$bol['usar_cli_tit'] = (isset($_GET['cli_tit'])) ? strval(urldecode($_GET['cli_tit'])) : 'CLI';
/**
...
/**
 * @var string - Nosso número (sem o DV - REGRA: Máximo de 7 caracteres).
 */
$bol["nosso_num"] = (isset($_GET['nosso_num'])) ? strval(urldecode($_GET['nosso_num'])) : '';
/**
...
/**
 * @var string - Data do Documento (usado também para gerar o campo nosso número).
 */
...
/**
 * @var string - Código do Cedente (PSK) (Somente 7 digitos).
 */
$bol['cod_ced'] = (isset($_GET['Cod_Ced'])) ? strval(urldecode($_GET['Cod_Ced'])) : '';
/**
 * @var string - Código do Cliente (PSK) (Somente 7 digitos).
 */
$bol['cod_cli'] = (isset($_GET['cod_cli'])) ? strval(urldecode($_GET['cod_cli'])) : '';
/**
 * @var string - Ponto de Venda = Agência.
 */
$bol['cod_ag'] = (isset($_GET['Cod_Ag'])) ? strval(urldecode($_GET['Cod_Ag'])) : '';
/**
 * @var string - Código da Carteira (102 Cobrança Simples - SEM Registro).
 */
$bol['cart_ced'] = (isset($_GET['Cart'])) ? strval(urldecode($_GET['Cart'])) : '';
/**
 * @var string - Descrição da carteira.
 */
$bol['desc_cart_ced'] = (isset($_GET['desc_cart_ced'])) ? strval(urldecode($_GET['desc_cart_ced'])) : '';
/**
...
$arr_data_doc = explode('/', $bol["data_doc"]);
$mes = $arr_data_doc[1];
$ano = $arr_data_doc[2];
$dadosboleto["nosso_numero"]        = $bol['num_doc'];
...
$dadosboleto["codigo_cliente"]      = $bol['cod_ced'];
$dadosboleto["ponto_venda"]         = $bol['cod_ag'];
$dadosboleto["carteira"]            = $bol['cart_ced'];
$dadosboleto["carteira_descricao"]  = $bol['desc_cart_ced'];
...
// NÃO ALTERAR!
include_once("BoletoSantander/include/funcoes_santander_banespa.php");
include_once("BoletoSantander/include/layout_santander_banespa.php");
