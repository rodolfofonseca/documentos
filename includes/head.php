...
<body>
    <script>
        /** 
         * Função responsável por pesquisar a notificacação no sistema, alterar a mesma para o status de leitura "LIDO" e retornar as informações para que o usuário possa estar visualizando de forma completa
        */
        function visualizar_notificacao(codigo_notificacao){
            sistema.request.post('/notificacoes.php', {'rota':'visualizar', 'codigo_notificacao':codigo_notificacao}, function(retorno){
                Swal.fire({'text':retorno.mensagem_longa});
            }, false);
        }
    </script>
                    ...
                    <ul class="navbar-nav float-left mr-auto ml-3 pl-1">
                        <li class="nav-item dropdown">
                            <a class="nav-link dropdown-toggle pl-md-3 position-relative" href="javascript:void(0)" id="bell" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                ...
                                <span class="badge badge-primary notify-no rounded-circle"><?php echo $quantidade_notificacao; ?></span>
                            </a>
                            <?php
                            if ($quantidade_notificacao > 0) {
                                if(empty($notificacoes) == false){
                                                ...
                                                <ul class="list-style-none">
                                                    <li>
                                                        <div class="message-center notifications position-relative">
                                                            <a href="#" onclick="visualizar_notificacao(<?php echo $notificacao['id_notificacao']; ?>);" class="message-item d-flex align-items-center border-bottom px-3 py-2">
                                                                <div class="btn btn-danger rounded-circle btn-circle">
                                                                    <i data-feather="<?php echo $notificacao['data_feather']; ?>" class="text-white"></i>
                                                                </div>
                                                                <div class="w-75 d-inline-block v-middle pl-2">
                                                                    <h6 class="message-title mb-0 mt-1"><?php echo $notificacao['titulo_notificacao']; ?></h6>
                                                                    <span class="font-12 text-nowrap d-block text-muted"><?php echo $notificacao['mensagem_curta']; ?></span>
                                                                    <span class="font-12 text-nowrap d-block text-muted"><?php echo convert_date($notificacao['data_notificacao']); ?></span>
                                                                </div>
                                                            </a>
                                                        </div>
                                                    </li>
