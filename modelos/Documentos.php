...
class Documentos
{
    ...
    private function colocar_dados($dados)
    {
        ...
        if (array_key_exists('login_usuario', $dados) == true) {
            $this->nome_usuario = (string) strtolower($dados['login_usuario']);
        }
    }
    ...
    private function return_diretorio($array_retorno)
    {
        ...
        // }
        if (is_dir($endereco_documento) == false) {
    ...
    private function salvar_api($arquivo)
    {
        ...
        if ($this->escolha_usuario == 'TODOS') {
            if ($checar_existencia == true) {
                return (bool) model_update($this->tabela(), ['id_documento', '===', (int) $this->id_documento], model_parse($this->modelo(), ['id_documento' => (int) $this->id_documento, 'id_caixa' => (int) $this->id_caixa, 'id_tipo_arquivo' => (int) $this->id_tipo_arquivo, 'id_organizacao' => (int) $this->id_organizacao, 'id_usuario' => (int) $id_usuario, 'nome_documento' => (string) $this->nome_documento, 'descricao' => (string) $this->descricao, 'endereco' => (string) $retorno_upload['url'], 'codigo_barras' => (string) $this->codigo_barras, 'quantidade_downloads' => (int) $this->quantidade_downloads, 'cloudinary' => (int) $retorno_upload['cloudinary'], 'data_alteracao' => model_date()]));
            } else {
    ...
    private function salvar_local($arquivo)
    {
        ...
        if (empty($retorno_extensao) == false) {
            ...
            if (@move_uploaded_file($nome_temporario, $nome_documento)) {
                ...
                if ($this->escolha_usuario == 'TODOS') {
                    if ($checar_existencia == true) {
                        return (bool) model_update($this->tabela(), ['id_documento', '===', (int) $this->id_documento], model_parse($this->modelo(), ['id_documento' => (int) $this->id_documento, 'id_caixa' => (int) $this->id_caixa, 'id_tipo_arquivo' => (int) $this->id_tipo_arquivo, 'id_organizacao' => (int) $this->id_organizacao, 'id_usuario' => (int) $id_usuario, 'nome_documento' => (string) $this->nome_documento, 'descricao' => (string) $this->descricao, 'endereco' => (string) $nome_documento, 'codigo_barras' => (string) $this->codigo_barras, 'quantidade_downloads' => (int) $this->quantidade_downloads, 'data_alteracao' => model_date()]));
                    } else {
