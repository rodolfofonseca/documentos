        ...
        if($checar_existencia == true){
            ...
            if($retorno_operacao == true){
                if(empty($retorno_usuario) == false){
                    ...
                    $retorno_notificacao = (array) $objeto_notificacao->salvar_dados((array) ['id_usuario' => (int) $this->id_usuario, 'id_empresa' => (int) $this->id_empresa, 'titulo_notificacao' => (string) 'Novo armario cadastrado', 'mensagem_curta' => (string) 'Um novo armário foi cadastrado no sistema!', 'mensagem_longa' => (string) 'O amário '.$this->nome_armario.' foi cadastrado no sistema, sendo possível cadastrar novas prateleiras!']);
                }
            }
        }
