...
class Contas{
    ...
    /**
     * Array responsável por realizar a validação das contas vencidas e retornar a quantidade de cada status de conta
     * @return array [contas_atrasadas, contas_aguardando, contas_pagas]
     */
    public function validar_vencimento_contas(){
        $data_hoje = (string) date('Y-m-d');
        $retorno_final = (array) ['contas_atrasadas' => (int) 0, 'contas_aguardando' => (int) 0, 'contas_pagas' => (int) 0];
        ...
        $retorno_contas_pagas = pesquisa_banco_aggregate($this->tabela(), (array) [['$match' => ['status' => 'PAGA']], ['$group' => ['_id' => [], 'COUNT(*)' => ['$sum' => 1]]], ['$project' => ['COUNT(*)' => '$COUNT(*)', '_id' => 0]]]);

        foreach($retorno_contas_pagas as $contas_pagas){
            foreach($contas_pagas as $quantidade){
                $retorno_final['contas_pagas'] = (int) intval($quantidade, 10);
            }
        }
